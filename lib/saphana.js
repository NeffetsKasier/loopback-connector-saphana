 ...
 */
/**
 * Increase the MAX_PACKET_SIZE for large base64 strings
 * 
 * Autor: Steffen Kaiser 
*/
require('hdb/lib/protocol/common/Constants').MAX_PACKET_SIZE = Math.pow(2,28);
var hdb = require('hdb');
...
/**
 * The customizing extends the seetings of the database connector with a certificate attribut, so that it is possible to implement a trusted connection
 * 
 * Autor: Steffen Kaiser 
*/
/**
 ...
 * Autor: Steffen Kaiser 
 * 
 * Using sql = 'SELECT current_identity_value()  AS "insertId" ' + 'FROM DUMMY'
 * instead of sql = SELECT this.tableEscaped(model).CURRVAL AS "insertId" FROM DUMMY
 */
SAPHANA.prototype.create = function (model, data, callback) {
    ...
    self.query(sql, fieldsAndData.params, function (err, results) {
        ...
        sql = 'SELECT current_identity_value()  AS "insertId" ' + 'FROM DUMMY' + SAPHANA.newline;
...
SAPHANA.prototype.toDatabase = function (prop, val) {
    ...
    /**
    *MODIFIED BY STEFFEN KAISER 
    * 
    *Return boolean objects instead of VARCHAR(1) --> 'X'
    *SAP HANA supports now boolean
    */
